...
`[1:]), 0666); err != nil {
		...
		config, err := ReadConfigFile(filename, false)
		if err != nil {
			t.Fatal(err)
...
func TestNewFileReplicaFromConfig(t *testing.T) {
	r, err := NewReplicaFromConfig(&ReplicaConfig{Path: "/foo"}, nil)
	if err != nil {
		t.Fatal(err)
...
func TestNewS3ReplicaFromConfig(t *testing.T) {
	t.Run("URL", func(t *testing.T) {
		r, err := NewReplicaFromConfig(&ReplicaConfig{URL: "s3://foo/bar"}, nil)
		if err != nil {
			t.Fatal(err)
	...
	t.Run("MinIO", func(t *testing.T) {
		r, err := NewReplicaFromConfig(&ReplicaConfig{URL: "s3://foo.localhost:9000/bar"}, nil)
		if err != nil {
			t.Fatal(err)
		} else if client, ok := r.Client().(*s3.ReplicaClient); !ok {
			t.Fatal("unexpected replica type")
	...
	t.Run("Backblaze", func(t *testing.T) {
		r, err := NewReplicaFromConfig(&ReplicaConfig{URL: "s3://foo.s3.us-west-000.backblazeb2.com/bar"}, nil)
		if err != nil {
			t.Fatal(err)
		} else if client, ok := r.Client().(*s3.ReplicaClient); !ok {
			t.Fatal("unexpected replica type")
...
func TestNewGSReplicaFromConfig(t *testing.T) {
	r, err := NewReplicaFromConfig(&ReplicaConfig{URL: "gs://foo/bar"}, nil)
	if err != nil {
		t.Fatal(err)
